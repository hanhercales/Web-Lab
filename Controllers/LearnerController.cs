...
public class LearnerController : Controller
{
    ...
    {
        if (id == null || db.Learners == null) return NotFound();
    ...
    public IActionResult Edit(int id, [Bind("LearnerID,FirstMidName,LastName,MajorID,EnrollmentDate")] Learner learner)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            {
                if (!LearnerExists(learner.LearnerID)) return NotFound();
                else throw;
            }
            return RedirectToAction(nameof(Index));
        }
    ...
    public IActionResult Delete(int id)
    {
        ...
        var learner = db.Learners.Include(l => l.Major)
            .Include(e => e.Enrollments).FirstOrDefault(m => m.LearnerID == id);
        if (learner == null) return NotFound();
        if(learner.Enrollments.Count > 0)
            return Content("This learner has enrollments, can't delete");
        return View(learner);
    }
    ...
    {
        if (db.Learners == null) return Problem("Entity set 'Learners' is null");
        var learner = db.Learners.Find(id);
        if(learner != null)
